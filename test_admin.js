  ...
  if (target) target.replaceWith(s);
    ...
    (function injectDESection() {
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;margin-bottom:22px;">
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
                ...
                <button onclick="adeAreaModal()" class="btn btn-small" style="background:#00796B;color:#fff;"><i class="fas fa-map-marker-alt" style="margin-right:6px;"></i>Manage Areas</button>
                ...
                <button onclick="adeExportExcel()" class="btn btn-small" style="background:#1D6F42;color:#fff;"><i class="fas fa-file-excel" style="margin-right:6px;"></i>Export Excel</button>
                <button onclick="adeLoad()" class="btn btn-small" style="background:#f4f4f4;color:#555;"><i class="fas fa-sync-alt"></i></button>
            </div>
        </div>
        ...
        <div id="adeFormCard" class="admin-card" style="display:none;padding:24px;margin-bottom:18px;border:1.5px solid var(--primary-color);">
            ...
            <form id="adeEntryForm" novalidate autocomplete="off">
                ...
                <div style="display:flex;gap:10px;margin-bottom:18px;flex-wrap:wrap;">
                    <label id="adeBtnInd" onclick="adeSetDonorType('Individual')" style="display:flex;align-items:center;gap:6px;padding:7px 18px;border:2px solid var(--primary-color);border-radius:20px;cursor:pointer;font-weight:600;font-size:.82rem;background:var(--primary-color);color:#fff;transition:all .2s;">
                        <i class="fas fa-user" style="font-size:.78rem;"></i> Individual
                    </label>
                    <label id="adeBtnBiz" onclick="adeSetDonorType('Business')" style="display:flex;align-items:center;gap:6px;padding:7px 18px;border:2px solid #ddd;border-radius:20px;cursor:pointer;font-weight:600;font-size:.82rem;background:#f9f9f9;color:#555;transition:all .2s;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:20px;">
                    <div class="form-group" style="margin:0;">
                        ...
                        <div style="display:flex;gap:8px;">
                            <select id="adeBuildingNameSelect" class="form-control" style="flex:1;"></select>
                    ...
                    <div class="form-group" style="margin:0;">
                        ...
                        <div style="display:flex;gap:8px;">
                            <select id="adeAreaSelect" class="form-control" style="flex:1;"></select>
                            <button type="button" onclick="adeAddAreaPrompt()" style="padding:0 14px;border:none;border-radius:8px;background:var(--primary-color);color:#fff;cursor:pointer;font-size:.85rem;white-space:nowrap;">＋ Add</button>
                        </div>
                    </div>
                    <div class="form-group" style="margin:0;">
                        <label for="adeLandmarkSelect">Common Landmark</label>
                        <select id="adeLandmarkSelect" class="form-control">
        ...
        <div class="admin-card" style="padding:24px;margin-bottom:18px;">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:18px;padding-bottom:14px;border-bottom:2px solid #F0F0F0;">
                ...
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                    ...
                    <input type="text" id="adeCardSearch" placeholder="🔍 Search donor, area…"
        ...
        <div class="admin-card" style="padding:20px;margin-bottom:18px;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;align-items:end;">
                ...
                <div>
                    <label style="font-size:.75rem;font-weight:700;text-transform:uppercase;color:#555;display:block;margin-bottom:5px;">Area</label>
                    <select id="adeFilterArea" class="form-control" onchange="adeFilter()" style="padding:9px 12px;"><option value="">All Areas</option></select>
                </div>
                <div>
                ...
                <table class="admin-table" id="adeTable">
                    <thead>
                        <tr>
                            ...
                            <th>WhatsApp</th><th>Mobile</th><th>Building</th><th>Area</th>
    ...
    function adeRenderCards() {
        ...
        const list = _adeAll.filter(e => {
            ...
            return donor.toLowerCase().includes(q) || (e.area || '').toLowerCase().includes(q);
        });
        ...
        const modeColor = { Cash:'#27AE60', Cheque:'#1565C0', UPI:'#6A1B9A', RTGS:'#00796B', Balance:'#E65100' };
        grid.innerHTML = list.slice().reverse().map(e => {
            ...
            return `<div style="background:#fff;border:1.5px solid #F0F0F0;border-radius:14px;padding:18px 16px;box-shadow:0 2px 8px rgba(0,0,0,.06);transition:box-shadow .2s;" onmouseover="this.style.boxShadow='0 4px 18px rgba(0,0,0,.11)'" onmouseout="this.style.boxShadow='0 2px 8px rgba(0,0,0,.06)'">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:10px;">
                    <span style="background:#F3E5F5;color:#6A1B9A;font-size:.73rem;font-weight:700;padding:3px 9px;border-radius:20px;"><i class="fas fa-book" style="margin-right:4px;"></i>Bk ${e.bookNumber} / #${e.receiptNumber}</span>
                    ${e.area ? `<span style="background:#E8F5E9;color:#1B5E20;font-size:.73rem;font-weight:700;padding:3px 9px;border-radius:20px;"><i class="fas fa-map-marker-alt" style="margin-right:4px;"></i>${e.area}</span>` : ''}
                    <span style="background:#FFF8E1;color:${modeClr};font-size:.73rem;font-weight:700;padding:3px 9px;border-radius:20px;">${mode}</span>
                </div>
    ...
    async function adeLoadDropdowns() {
        await Promise.all([adeLoadBuildings(), adeLoadAreas(), adeLoadLandmarks()]);
    }
    ...
    async function adeAddAreaPrompt() {
        ...
        try {
            ...
            if (r.ok && d.success) {
                await adeLoadAreas();
                document.getElementById('adeAreaSelect').value = d.area.name;
            } else {
                alert(d.message || 'Could not add area.');
            }
        } catch (e) {
            alert('Server error.');
        }
    ...
    async function adeLoad() {
        ...
        if (!tbody) return;
        tbody.innerHTML = '<tr><td colspan="17" style="text-align:center;color:#999;padding:30px;">Loading…</td></tr>';
        try {
            ...
            const areaSel = document.getElementById('adeFilterArea');
            if (areaSel) {
                areaSel.innerHTML = '<option value="">All Areas</option>';
                (areaData.areas || []).forEach(a => areaSel.innerHTML += `<option>${a.name}</option>`);
            }
            adeFilter();
            adeRenderCards();
        } catch(e) {
    ...
    function adeFilter() {
        ...
        const area = document.getElementById('adeFilterArea')?.value;
    ...
    function adeClearFilters() {
        ...
        ['adeFilterArea','adeFilterMode','adeFilterType'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
        adeFilter();
    }
    ...
    function adeRender() {
        ...
        tbody.innerHTML = _adeFiltered.slice().reverse().map((e, i) => {
            ...
            return `<tr id="ade-row-${e.entryId}">
                ...
                <td style="font-size:.85rem;">${e.buildingName||'—'}</td>
                <td style="font-size:.85rem;">${e.area||'—'}</td>
                <td style="color:#2E7D32;font-weight:700;">${amt}</td>
                <td><span style="padding:3px 10px;border-radius:12px;background:#FFF3E0;color:#E65100;font-size:.75rem;font-weight:700;">${e.paymentMode}</span></td>
    ...
    function adeOpenEdit(id) {
        ...
        modal.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:30px 28px;max-width:680px;width:94%;max-height:90vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.25);">
            ...
            <form onsubmit="adeSaveEdit(event)" autocomplete="off">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                    ...
                    <div class="form-group"><label>Area</label><input name="area" class="form-control" value="${e.area||''}"></div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
    ...
    // ── Building/Area Management modals ──────────────────────────
    async function adeBuildingModal() {
        ...
        modal.style.cssText = 'display:flex;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center;';
        modal.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:28px;max-width:440px;width:90%;max-height:80vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.2);">
            ...
            <div style="display:flex;gap:8px;margin-bottom:16px;">
                <input type="text" id="adeBldgInput" class="form-control" placeholder="New building name" style="flex:1;">
                <button onclick="adeAddBldg()" class="btn btn-primary btn-small">Add</button>
            </div>
            <div id="adeBldgList" style="display:flex;flex-direction:column;gap:8px;">
                ${(data.buildings||[]).map(b => `
                <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f8f9fa;border-radius:8px;">
                    <span style="flex:1;font-weight:600;">${b.name}</span>
                    <button onclick="adeDeleteBldg('${b.id}',this)" style="border:none;background:#FFEBEE;color:#c0392b;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:.8rem;"><i class="fas fa-trash"></i></button>
                </div>`).join('')}
            </div>
        </div>`;
        document.body.appendChild(modal);
    ...
    async function adeAddBldg() {
        const name = document.getElementById('adeBldgInput')?.value.trim();
        if (!name) return;
        const res  = await fetch('/api/buildings', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name }) });
        const data = await res.json();
        if (res.ok && data.success) { document.getElementById('adeBldgModal')?.remove(); adeBuildingModal(); }
        else alert(data.message || 'Could not add.');
    }
    ...
    async function adeAreaModal() {
        const res  = await fetch('/api/areas');
        const data = await res.json();
        const modal = document.createElement('div');
        modal.id = 'adeAreaModalEl';
        modal.style.cssText = 'display:flex;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center;';
        modal.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:28px;max-width:440px;width:90%;max-height:80vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.2);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
                <h3 style="margin:0;"><i class="fas fa-map-marker-alt" style="color:#00796B;margin-right:8px;"></i>Manage Areas</h3>
                <span onclick="document.getElementById('adeAreaModalEl').remove()" style="font-size:1.5rem;cursor:pointer;color:#999;">&times;</span>
            </div>
            <div style="display:flex;gap:8px;margin-bottom:16px;">
                <input type="text" id="adeAreaInput" class="form-control" placeholder="New area name" style="flex:1;">
                <button onclick="adeAddArea()" class="btn btn-primary btn-small">Add</button>
            </div>
            <div id="adeAreaList" style="display:flex;flex-direction:column;gap:8px;">
                ${(data.areas||[]).map(a => `
                <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f8f9fa;border-radius:8px;">
                    <span style="flex:1;font-weight:600;">${a.name}</span>
                    <button onclick="adeDeleteArea('${a.id}',this)" style="border:none;background:#FFEBEE;color:#c0392b;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:.8rem;"><i class="fas fa-trash"></i></button>
                </div>`).join('')}
            </div>
        </div>`;
        document.body.appendChild(modal);
    ...
    async function adeAddArea() {
        const name = document.getElementById('adeAreaInput')?.value.trim();
        if (!name) return;
        const res  = await fetch('/api/areas', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name }) });
        const data = await res.json();
        if (res.ok && data.success) { document.getElementById('adeAreaModalEl')?.remove(); adeAreaModal(); }
        else alert(data.message || 'Could not add.');
    }

    async function adeDeleteArea(id, btn) {
    ...
    (function injectAdminNewEntryForm() {
        ...
        formCard.style.cssText = 'background:#fff;border-radius:14px;box-shadow:0 2px 12px rgba(0,0,0,.08);margin-bottom:22px;overflow:hidden;';
        formCard.innerHTML = `
            ...
            <form id="aneForm" onsubmit="aneSave(event)" autocomplete="off">
                ...
                <div class="form-group" style="margin-bottom:14px;">
                  ...
                  <div style="display:flex;gap:10px;margin-top:6px;">
                    <button type="button" id="aneBtnInd" onclick="aneSetType('Individual')" style="padding:7px 20px;border:2px solid var(--primary-color);border-radius:20px;background:var(--primary-color);color:#fff;font-weight:700;cursor:pointer;">Individual</button>
                    <button type="button" id="aneBtnBiz" onclick="aneSetType('Business')" style="padding:7px 20px;border:2px solid #ddd;border-radius:20px;background:#f9f9f9;color:#555;font-weight:700;cursor:pointer;">Business</button>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
                    ...
                    <div class="form-group" style="margin:0;"><label>Building Name</label><select id="aneBuilding" class="form-control"><option value="">— Select Building —</option></select></div>
                    <div class="form-group" style="margin:0;"><label>Common Landmark</label><select id="aneLandmark" class="form-control"><option value="">— Select Landmark —</option></select></div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
    ...
    const _aneModeColors = {
        ...
        RTGS    : { bg:'#00796B',             border:'#00796B',              color:'#fff' },
    ...
    function aneToggle() {
        ...
        if (_aneOpen) { anePopulateBooks(); aneLoadAreas(); }
    }
    ...
    async function aneLoadAreas() {
        try { const r=await fetch('/api/areas'); const d=await r.json(); const sel=document.getElementById('aneArea'); if(!sel) return; (d.areas||[]).forEach(a=>{ if(!['Patelwadi','Shindewadi','Gurkhawadi'].includes(a.name)){ const o=document.createElement('option'); o.value=a.name; o.textContent=a.name; sel.appendChild(o); } }); } catch(e){}
    }
    async function aneBookChange() {
        const bn = Number(document.getElementById('aneBook').value);
        const sel = document.getElementById('aneReceipt');
        if (!bn) { sel.innerHTML='<option>— Select Book first —</option>'; sel.disabled=true; return; }
        ...
        const from=(bn-1)*50+1, to=bn*50;
        sel.innerHTML='<option value="">— Select Receipt —</option>';
        for(let n=from;n<=to;n++){const taken=used.includes(n);const o=document.createElement('option');o.value=n;o.textContent=n+(taken?' (used)':'');if(taken)o.disabled=true;sel.appendChild(o);}
        sel.disabled=false;
    }
    ...
    function aneReset() {
        ...
        const rs=document.getElementById('aneReceipt'); if(rs){rs.innerHTML='<option>\u2014 Select Book first \u2014</option>';rs.disabled=true;}
        const pp=document.getElementById('anePhotoPreview'); if(pp) pp.style.display='none';
        _anePhotoFile=null;
        window._anePhotoFile=null;
        document.getElementById('anePhotoInput').value='';
        document.getElementById('aneStatus').style.display='none';
        // Reset payment mode tiles back to Cash
        aneSetMode('Cash');
        const rg=document.getElementById('aneRefGroup'); if(rg) rg.style.display='';
    }
    async function aneSave(ev) {
        ev.preventDefault();
        ...
        if(!fn||!mn||!ln){aneShowStatus('\u274c First, Middle, and Last Name are required.','error');if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane" style="margin-right:6px;"></i>Save Entry';}return;}
        const currentUser = JSON.parse(sessionStorage.getItem('currentUser')||'null');
        const payload={
            bookNumber:bn, receiptNumber:rn, donorType:'Individual',
        ...
        try {
            ...
            if(res.ok&&data.success){
                if(window._anePhotoFile&&data.entry&&data.entry.entryId){
                    try{
                        const fd=new FormData();
                        fd.append('passbook',window._anePhotoFile,'receipt.jpg');
                        fd.append('entryId',data.entry.entryId);
                        fd.append('userId',currentUser?String(currentUser.id):'');
                        await fetch('/api/upload-passbook',{method:'POST',body:fd});
                    }catch(_px){} finally{ window._anePhotoFile=null; }
                }
    ...
    // ── Donor type toggle for admin new entry form ───────────────────────────
    function aneSetType(type) {
        document.getElementById('aneDonorType').value = type;
        const isInd = type === 'Individual';
        const actS = 'padding:7px 20px;border:2px solid var(--primary-color);border-radius:20px;background:var(--primary-color);color:#fff;font-weight:700;cursor:pointer;';
        const inactS = 'padding:7px 20px;border:2px solid #ddd;border-radius:20px;background:#f9f9f9;color:#555;font-weight:700;cursor:pointer;';
        document.getElementById('aneBtnInd').style.cssText = isInd ? actS : inactS;
        document.getElementById('aneBtnBiz').style.cssText = !isInd ? actS : inactS;
        document.getElementById('aneIndFields').style.display = isInd ? 'grid' : 'none';
        document.getElementById('aneBizFields').style.display = !isInd ? 'block' : 'none';
    }

    // ── Email hint auto-append @gmail.com ───────────────────────────────────
    function aneEmailHint(inp) {
        const hint = document.getElementById('aneEmailHint');
        if (!inp.value.includes('@') && inp.value.trim()) {
            if (hint) hint.textContent = inp.value.trim() + '@gmail.com';
        } else { if (hint) hint.textContent = ''; }
    }

    // ── Auto-receipt: pick next free slot ───────────────────────────────────
    async function aneAutoReceipt() {
        try {
            const resp = await fetch('/api/donation-entries/next-receipt');
            const slot = await resp.json();
            if (!slot.bookNumber) { alert('All receipt numbers are used.'); return; }
            const bookSel = document.getElementById('aneBook');
            if (bookSel) bookSel.value = slot.bookNumber;
            await aneBookChange();
            const receiptSel = document.getElementById('aneReceipt');
            if (receiptSel) receiptSel.value = slot.receiptNumber;
            aneShowStatus('✅ Auto-assigned: Book ' + slot.bookNumber + ', Receipt #' + slot.receiptNumber, 'success');
        } catch(e) { alert('Could not fetch next receipt: ' + e.message); }
    }

    // ── Load buildings into ane form ────────────────────────────────────────
    async function aneLoadBuildings() {
        try {
            const r = await fetch('/api/buildings'); const d = await r.json();
            const sel = document.getElementById('aneBuilding'); if (!sel) return;
            const cur = sel.value;
            sel.innerHTML = '<option value="">— Select Building —</option>';
            (d.buildings||[]).forEach(b => sel.innerHTML += `<option value="${b.name}" ${b.name===cur?'selected':''}>${b.name}</option>`);
        } catch(e) {}
    }

    // ── Load landmarks into ane form ────────────────────────────────────────
    async function aneLoadLandmarks() {
        try {
            const r = await fetch('/api/landmarks'); const d = await r.json();
            const sel = document.getElementById('aneLandmark'); if (!sel) return;
            const cur = sel.value;
            sel.innerHTML = '<option value="">— Select Landmark —</option>';
            (d.landmarks||[]).forEach(l => sel.innerHTML += `<option value="${l.name}" ${l.name===cur?'selected':''}>${l.name}</option>`);
        } catch(e) {}
    }
    ...
    async function adeLandmarkModal() {
        ...
        modal.id = 'adeLmModal';
        modal.style.cssText = 'display:flex;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center;';
        modal.innerHTML = `<div style="background:#fff;border-radius:16px;padding:28px;max-width:440px;width:90%;max-height:80vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.2);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
    ...
    (function injectCommitteeSection() {
        ...
        sec.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;margin-bottom:22px;">
            <div><h2 style="color:var(--dark-color);margin:0 0 4px;">Committee Members</h2><p style="color:#777;margin:0;">Manage committee — visible to all volunteers</p></div>
    ...
    async function cmLoad() {
        ...
        grid.innerHTML = '<div style="text-align:center;color:#aaa;padding:40px;grid-column:1/-1;">Loading…</div>';
        try {
            ...
            grid.innerHTML = _cmAll.map(m => `<div style="background:#fff;border-radius:14px;box-shadow:0 2px 12px rgba(0,0,0,.08);overflow:hidden;text-align:center;">
                ${m.photoUrl?
                    `<img src="${fixUrl(m.photoUrl)}" style="width:100%;height:160px;object-fit:cover;" onclick="openAdminLightbox('${fixUrl(m.photoUrl)}')">`:
                    `<div style="width:100%;height:160px;background:linear-gradient(135deg,var(--primary-color),#ff8c42);display:flex;align-items:center;justify-content:center;"><i class="fas fa-user" style="font-size:3rem;color:#fff;opacity:.5;"></i></div>`}
                <div style="padding:14px 12px 16px;">
                    <div style="font-weight:700;font-size:1rem;margin-bottom:4px;">${m.name}</div>
                    <div style="font-size:.82rem;color:var(--primary-color);font-weight:600;margin-bottom:6px;">${m.role||m.department||'Member'}</div>
                    ${m.phone?'<div style="font-size:.8rem;color:#666;margin-bottom:4px;"><i class="fas fa-phone" style="margin-right:4px;"></i>'+m.phone+'</div>':''}
                    ${m.memberId?'<div style="font-size:.78rem;color:#aaa;margin-bottom:8px;">ID: '+m.memberId+'</div>':''}
                    <div style="display:flex;gap:8px;justify-content:center;margin-top:8px;">
                        <button onclick="cmOpenEdit('${m.id}')" style="padding:5px 12px;border:none;border-radius:8px;background:#E3F2FD;color:#1565C0;cursor:pointer;font-size:.8rem;"><i class="fas fa-edit"></i></button>
                        <button onclick="cmDelete('${m.id}')" style="padding:5px 12px;border:none;border-radius:8px;background:#FFEBEE;color:#c0392b;cursor:pointer;font-size:.8rem;"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
            </div>`).join('');
    ...
    function cmOpenModal(member) {
        ...
        modal.id = 'cmModal';
        modal.style.cssText = 'display:flex;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center;';
        modal.innerHTML = `<div style="background:#fff;border-radius:16px;padding:28px 24px;max-width:480px;width:94%;max-height:88vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.25);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
                <h3 style="margin:0;">${isEdit?'Edit':'Add'} Committee Member</h3>
            ...
            <div class="form-group"><label>Full Name *</label><input id="cmName" class="form-control" value="${member?member.name:''}"></div>
            <div class="form-group"><label>Member ID</label><input id="cmMemberId" class="form-control" value="${member?member.memberId:''}"></div>
            <div class="form-group"><label>Phone Number</label><input id="cmPhone" class="form-control" maxlength="10" value="${member?member.phone:''}"></div>
            <div class="form-group"><label>Department</label><input id="cmDept" class="form-control" value="${member?member.department:''}"></div>
            <div class="form-group"><label>Role / Position</label><input id="cmRole" class="form-control" value="${member?member.role:''}"></div>
            <div style="border:1.5px dashed #ffe0d0;border-radius:10px;padding:14px;margin-bottom:16px;">
                ...
                ${member&&member.photoUrl?`<img src="${fixUrl(member.photoUrl)}" style="width:80px;height:80px;object-fit:cover;border-radius:50%;border:2px solid #ffe0d0;margin-bottom:10px;">`:''}
                <button type="button" onclick="document.getElementById('cmPhotoInput').click()" style="width:100%;padding:10px;border:2px dashed #ddd;border-radius:10px;background:#fff8f5;color:#555;cursor:pointer;font-size:.85rem;font-weight:600;"><i class="fas fa-upload" style="margin-right:6px;color:#E65100;"></i>Upload Photo</button>
                <input type="file" id="cmPhotoInput" style="display:none;" accept="image/*">
                <div id="cmPhotoPreview" style="display:none;margin-top:10px;">
                    <img id="cmPhotoThumb" src="" style="width:80px;height:80px;object-fit:cover;border-radius:50%;border:2px solid #ffe0d0;">
                    <span id="cmPhotoName" style="font-size:.8rem;color:#555;margin-left:10px;"></span>
                </div>
            </div>
            <div style="display:flex;gap:10px;justify-content:flex-end;">
        ...
        document.getElementById('cmPhotoInput').onchange = function(ev) {
            ...
            r.onload = function(re) {
                ...
                document.getElementById('cmPhotoName').textContent = f.name;
    ...
    async function cmSave(editId) {
        ...
        const payload = {
            name, memberId:document.getElementById('cmMemberId')?.value.trim()||'',
        ...
        try {
            ...
            if(_cmPhotoFile){
                try{
                    const fd=new FormData();fd.append('photo',_cmPhotoFile,_cmPhotoFile.name);fd.append('memberId',memberId);
