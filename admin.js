...
function openBrEditModal(receiptId, name, amount, mode, status, bookNum, receiptNum, photoUrl, type, extraBookId) {
    ...
    if (inp) {
        inp.value = ''; inp.onchange = function (ev) {
...
async function volToggleBlock(id) {
    const bl = _volGetBlocklist();
    ...
    // Persist to server
    const user = _VOL_USERS.find(u => u.id === id);
    if (user && user.username) {
        try {
            await fetch('/api/users/block', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: user.username, blocked: willBlock })
            });
        } catch (e) {
            console.warn('Could not persist block status to server:', e.message);
        }
    }
