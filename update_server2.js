...
function applyServerChanges() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (lines[i].includes('const bn = Number(bookNumber);') && lines[i+1].includes('const rn = Number(receiptNumber);')) {
            ...
            } else if (lines[i+2].includes('if (!bn || bn < 1 || bn > TOTAL_BOOKS_DE)') || (lines[i+3] && lines[i+3].includes('TOTAL_BOOKS_DE'))) {
                // PUT API Block
                // Let's just find "const bn = Number(bookNumber);" inside PUT API.
            }
        }
